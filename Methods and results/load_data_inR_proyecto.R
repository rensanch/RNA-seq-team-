...
# --- Load data -----
# Cargar archivos
indir <- "/mnt/Guanina/bioinfo24/Equipo4/proyecto/STAR_output/"
...
# Cargar Metadatos
metadata <- read.csv("/mnt/Guanina/bioinfo24/Equipo4/proyecto/DEG_results/metadataBreastCancer.csv", header = F)
...
colnames(counts) <- sub("ReadsPerGene.out.tab", "", files) #Quitamos ReadsPerGene.out.tab de todas las columnas 

# Eliminar las 4 primeras filas -> las filas con informacion del alineamiento
# counts <- counts[5:129239, ] # Filtramos los rows con informacion general sobre el mapeo
counts <- counts[-c(1:4),]

# Almacenar metadata y matriz de cuentas
save(metadata, counts, file = paste0(outdir, "raw_counts.RData"))
write.csv(counts, file = paste0(outdir,"raw_counts.csv"))
